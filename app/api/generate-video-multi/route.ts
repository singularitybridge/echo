...
import { generateVideo as generateVideoGemini } from '@/services/geminiService';
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Validate model IDs
    const validModels: VideoGenerationModel[] = ['veo-3.1', 'wan-2.5-i2v'];
    ...
    // Check if Veo is requested but API key is missing
    if (models.includes('veo-3.1') && !geminiApiKey) {
      return NextResponse.json(
        { error: 'NEXT_PUBLIC_GEMINI_API_KEY not configured for Veo 3.1' },
        { status: 500 }
      );
    }

    // Check if wan is requested but API key is missing
    if (models.includes('wan-2.5-i2v') && !falKey) {
      return NextResponse.json(
        { error: 'FAL_KEY not configured for wan-2.5-i2v' },
        { status: 500 }
      );
    }
      ...
      models.map(async (modelId): Promise<VideoGenerationResult> => {
        ...
        try {
          if (modelId === 'veo-3.1') {
            ...
            return {
              model: 'veo-3.1',
              videoUrl: result.uri, // Remote URL from Gemini (not blob URL)
